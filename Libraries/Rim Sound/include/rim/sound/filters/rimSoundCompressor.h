/*
 *  rimSoundCompressor.h
 *  Rim Sound
 *
 *  Created by Carl Schissler on 8/3/12.
 *  Copyright 2012 __MyCompanyName__. All rights reserved.
 *
 */

#ifndef INCLUDE_RIM_SOUND_COMPRESSOR_H
#define INCLUDE_RIM_SOUND_COMPRESSOR_H
...
//********************************************************************************
//********************************************************************************
/// A class which reduces the level of sound above a certain threshold.
/**
...
class Compressor : public SoundFilter
{
	public:
		...
		//******	Constructors
			...
			/// Create a new compressor with specified threshold, ratio, attack, and release.
			/**
		...
		//******	Ratio Accessor Methods
			...
			/**
			  ...
			  */
			RIM_INLINE Float getRatio() const
			{
				return targetRatio;
			}
			...
			/**
			  ...
			  */
			RIM_INLINE void setRatio( Float newRatio )
			{
				lockMutex();
		...
		//******	Knee Accessor Methods
			...
			/**
			  ...
			  */
			RIM_INLINE void setKnee( Gain newKnee )
			{
				lockMutex();
		...
		//******	Attack Accessor Methods
			...
			/**
			  ...
			  */
			RIM_INLINE Float getAttack() const
			{
				return attack;
			}
			...
			/**
			  ...
			  */
			RIM_INLINE void setAttack( Float newAttack )
			{
				lockMutex();
		...
		//******	Release Accessor Methods
			...
			/**
			  ...
			  */
			RIM_INLINE Float getRelease() const
			{
				return release;
			}
			...
			/**
			  ...
			  */
			RIM_INLINE void setRelease( Float newRelease )
			{
				lockMutex();
		...
		//******	RMS Time Accessor Methods
			...
			/**
			  ...
			  */
			RIM_INLINE Float getRMSTime() const
			{
				return rmsTime;
			}
			...
			/**
			  ...
			  */
			RIM_INLINE void setRMSTime( Float newRMSTime )
			{
				lockMutex();
		...
		//******	Channel Link Status Accessor Methods
			...
			/**
			  ...
			  */
			RIM_INLINE Bool getChannelsAreLinked() const
			{
				return linkChannels;
			}
			...
			/**
			  ...
			  * to maintain the stereo image of the audio when compressing hard-panned sounds.
			  */
			RIM_INLINE void setChannelsAreLinked( Bool newChannelsAreLinked )
			{
				lockMutex();
		...
		//******	Gain Reduction Accessor Methods
			...
			/**
			  ...
			  */
			RIM_INLINE Gain getGainReductionDB() const
			{
			...
			/**
			  ...
			  * compressor is compressing at any given time.
			  */
			RIM_INLINE Gain getGainReduction() const
			{
				return util::dbToLinear( currentReduction );
			}
		...
		//******	Transfer Function Accessor Methods
			...
			/// Evaluate the transfer function of the compressor for an envelope with the specified amplitude.
			RIM_INLINE Gain evaluateTransferFunctionDB( Gain input ) const
			{
				return util::linearToDB( this->evaluateTransferFunction( util::dbToLinear( input ) ) );
			}
	...
	private:
		...
		//******	Private Filter Processing Methods
			...
			RIM_FORCE_INLINE void compress( const SoundBuffer& inputBuffer, SoundBuffer& outputBuffer, Size numSamples,
			...
			RIM_FORCE_INLINE static Gain getDBReduction( Float level, Gain threshold, Float ratio,
														Float kneeMin, Float kneeMax, Float knee )
			{
				Float reductionConstant = (Float(1) - ratio)/ratio;
				Gain dbOver = util::linearToDB( level / threshold );
			...
			/// Return the negative gain reduction in decibels for the specified signal level and compression parameters.
			RIM_FORCE_INLINE static Gain getDBReduction2( Float level, Gain threshold, Float reductionConstant,
														Float kneeMin, Float kneeMax, Float knee )
			{
				Gain dbOver = util::linearToDB( level / threshold );
		...
		//******	Private Data Members
			...
			/**
			  ...
			  * compressed signal which is sent to the final output buffer. Thus, a mix factor
...
#endif // INCLUDE_RIM_SOUND_COMPRESSOR_H
