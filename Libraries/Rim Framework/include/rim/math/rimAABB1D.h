...
#ifndef INCLUDE_RIM_AABB_1D_H
#define INCLUDE_RIM_AABB_1D_H
...
class AABB1D
{
	public:
		...
		//******	Constructors
			...
			RIM_FORCE_INLINE AABB1D( T center )
				:	min( center ),
					max( center )
			{
			}
			...
			RIM_FORCE_INLINE AABB1D( T newMin, T newMax )
				:	min( newMin ),
					max( newMax )
			{
			}
		...
		//******	AABB Cast Operator
			...
			template < typename U >
			RIM_FORCE_INLINE operator AABB1D<U> () const
			{
		...
		//******	AABB Comparison Methods
			...
			RIM_FORCE_INLINE Bool contains( const AABB1D& bounds ) const
			{
			...
			RIM_FORCE_INLINE Bool contains( T value ) const
			{
			...
			RIM_FORCE_INLINE Bool intersects( const AABB1D& bounds ) const
			{
		...
		//******	Accessor Methods
			...
			RIM_FORCE_INLINE void set( T newMin, T newMax )
			{
				min = newMin;
				max = newMax;
			}
			...
			/// Get the difference between the maximum and minimum coordinates.
			RIM_FORCE_INLINE T getSize() const
			{
				return max - min;
			}
			...
			RIM_FORCE_INLINE T getCenter() const
			{
			...
			/**
			  ...
			  * result is undefined.
			  */
			RIM_FORCE_INLINE T getMinMax( Index i ) const
			{
				return (&min)[i];
			}
		...
		//******	Enlargement Methods
			...
			RIM_FORCE_INLINE void enlargeFor( T value )
			{
			...
			RIM_FORCE_INLINE void enlargeFor( const AABB1D& box )
			{
			...
			/// Modify the current bounding box such that it encloses the specified value.
			RIM_FORCE_INLINE AABB1D<T>& operator |= ( T value )
			{
				min = math::min( min, value );
				max = math::max( max, value );
			...
			RIM_FORCE_INLINE AABB1D<T> operator | ( T value ) const
			{
		...
		//******	Union Methods
			...
			RIM_FORCE_INLINE AABB1D<T> getUnion( const AABB1D<T>& bounds ) const
			{
			...
			RIM_FORCE_INLINE AABB1D<T>& operator |= ( const AABB1D<T>& bounds )
			{
			...
			/// Return the union of this bounding box and another.
			RIM_FORCE_INLINE AABB1D<T> operator | ( const AABB1D<T>& bounds ) const
			{
		...
		//******	Intersection Methods
			...
			RIM_FORCE_INLINE AABB1D<T> getIntersection( const AABB1D<T>& bounds ) const
			{
		...
		//******	Comparison Operators
			...
			RIM_INLINE Bool operator == ( const AABB1D<T>& other ) const
			{
