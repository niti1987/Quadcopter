 ...
 * 
 * File:        rim/math/SIMDScalarInt4.h
 * 
 * Version:     1.0.0
 * 
 * Contents:    rim::math::SIMDScalar class specialization for an 4-integer SIMD data type.
 * 
 * License:
 * 
...
#ifndef INCLUDE_RIM_SIMD_SCALAR_INT_4_H
#define INCLUDE_RIM_SIMD_SCALAR_INT_4_H
...
class RIM_ALIGN(16) SIMDScalar<Int32,4>
{
	private:
		...
		//******	Private Type Declarations
			...
			typedef RIM_ALTIVEC_VECTOR int SIMDScalar4;
	...
	public:
		...
		//******	Load Methods
...
#elif RIM_USE_SIMD && defined(RIM_SIMD_SSE) && RIM_SSE_VERSION_IS_SUPPORTED(1,0)
				return SIMDScalar( _mm_load_si128( (const SIMDScalar4*)array ) );
#else
				return SIMDScalar( array[0], array[1], array[2], array[3] );
#endif
			}
			...
			RIM_FORCE_INLINE static SIMDScalar loadUnaligned( const Int32* array )
			{
				...
				return SIMDScalar( array[0], array[1], array[2], array[3] );
#elif RIM_USE_SIMD && defined(RIM_SIMD_SSE) && RIM_SSE_VERSION_IS_SUPPORTED(1,0)
				return SIMDScalar( _mm_loadu_si128( (const SIMDScalar4*)array ) );
#else
				return SIMDScalar( array[0], array[1], array[2], array[3] );
#endif
			}
		...
		//******	Store Method
			...
			RIM_FORCE_INLINE void storeUnaligned( Int32* destination ) const
			{
				...
				destination[3] = d;
#elif RIM_USE_SIMD && defined(RIM_SIMD_SSE) && RIM_SSE_VERSION_IS_SUPPORTED(1,0)
				_mm_storeu_si128( (SIMDScalar4*)destination, v );
#else
				destination[0] = a;
				destination[1] = b;
				destination[2] = c;
		...
		//******	Logical Operators
			...
			RIM_FORCE_INLINE SIMDScalar operator & ( const SIMDScalar<Bool,4>& vector ) const
			{
#if RIM_USE_SIMD && defined(RIM_SIMD_ALTIVEC)
				return SIMDScalar( vec_and( v, vector.v ) );
#elif RIM_USE_SIMD && defined(RIM_SIMD_SSE) && RIM_SSE_VERSION_IS_SUPPORTED(2,0)
		...
		//******	Comparison Operators
			...
			/**
			  ...
			  * component is non-zero. Otherwise, that result component is equal to zero.
			  */
			RIM_FORCE_INLINE SIMDScalar<Bool,4> operator == ( const Int32 value ) const
			{
#if RIM_USE_SIMD && defined(RIM_SIMD_ALTIVEC)
...
RIM_FORCE_INLINE SIMDScalar<Int32,4> select( const SIMDScalar<Bool,4>& selector,
...
#endif // INCLUDE_RIM_SIMD_SCALAR_INT_4_H
