...
public:
    ...
    Vehicle(VehicleState startingState) {
		...
		currentVelocityBodyFrame = VehicleAttitudeHelpers
                .translateVectorToBodyFrame(VehicleAttitudeHelpers
	...
	Vector3f getGravityVector() {
		Vector3f gravityBodyFrame = Vector3f();
			...
			gravityBodyFrame = VehicleAttitudeHelpers
					.translateVectorToBodyFrame(VehicleAttitudeHelpers
							.getAttitudeMatrix(yaw, pitch, roll),
							GRAVITY_VECTOR);
	...
	Vector3f getNegativeZAxis() {
		Vector3f returnNegativeZAxis = Vector3f();
			...
			returnNegativeZAxis = VehicleAttitudeHelpers
					.getAttitudeMatrix(yaw, pitch, roll)
					.getColumn(VehicleAttitudeHelpers.INDEX_Z).negate();
...
/*
    ...
    void setNewOrientation(Vector3f thrustVector) {
        ...
        Vector3f yawPitchRollGlobalFrame = VehicleAttitudeHelpers
                .getPitchAndRollFromZWhenYawIsZero(positiveZAxis);

        VehicleState newState = VehicleState();
		newState.deepCopy(getVehicleState());

        newState.orientation = Orientation.buildOrientationFromRadians(
    ...
    void calculateAngularVelocities(double timeStep,
        ...
        Vector3f yawPitchRollBodyFrame = VehicleAttitudeHelpers
