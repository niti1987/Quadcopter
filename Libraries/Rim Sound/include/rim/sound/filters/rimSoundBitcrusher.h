/*
 *  rimSoundBitcrusher.h
 *  Rim Sound
 *
 *  Created by Carl Schissler on 8/6/13.
 *  Copyright 2013 __MyCompanyName__. All rights reserved.
 *
 */

#ifndef INCLUDE_RIM_SOUND_BITCRUSHER_H
#define INCLUDE_RIM_SOUND_BITCRUSHER_H
...
class Bitcrusher : public SoundFilter
{
	public:
		...
		//******	Clipping Mode Accessor Methods
			...
			/**
			  ...
			  */
			RIM_INLINE ClipMode getClipMode() const
			{
				return clipMode;
			}
			...
			/**
			  * This value determines what happens when the waveform exceeds the clipping threshold.
			  */
			RIM_INLINE void setClipMode( ClipMode newClipMode )
			{
				lockMutex();
		...
		//******	Bit Reduction Accessor Methods
			...
			RIM_INLINE Bool getBitReductionIsEnabled() const
			{
			...
			RIM_INLINE void setBitReductionIsEnabled( Bool newBitReduceEnabled )
			{
				lockMutex();
			...
			/**
			  ...
			  */
			RIM_INLINE UInt getBitResolution() const
			{
			...
			/**
			  ...
			  */
			RIM_INLINE void setBitResolution( UInt newResolution )
			{
				lockMutex();
		...
		//******	Dithering Accessor Methods
			...
			RIM_INLINE Bool getDitherIsEnabled() const
			{
			...
			RIM_INLINE void setDitherIsEnabled( Bool newDitherEnabled )
			{
				lockMutex();
			...
			/**
			  ...
			  * should be applied.
			  */
			RIM_INLINE Float getDitherAmount() const
			{
				return targetDither;
			}
			...
			/**
			  ...
			  */
			RIM_INLINE void setDitherAmount( Float newDither )
			{
				lockMutex();
		...
		//******	Downsampling Accessor Methods
			...
			/**
			  ...
			  * rate is halved, etc.
			  */
			RIM_INLINE UInt getDownsampling() const
			{
				return downsampling;
			}
			...
			/**
			  ...
			  */
			RIM_INLINE void setDownsampling( UInt newDownsampling )
			{
				lockMutex();
	...
	private:
		...
		//******	Private Clipping Functions
			...
			RIM_FORCE_INLINE static Float clipHard( Float input, Float threshold )
			{
				if ( input > threshold )
					return threshold;
			...
			RIM_FORCE_INLINE static Float clipInvert( Float input, Float threshold )
			{
			...
			RIM_FORCE_INLINE static Float clipWrap( Float input, Float threshold )
			{
				if ( input > threshold )
...
#endif // INCLUDE_RIM_SOUND_BITCRUSHER_H
