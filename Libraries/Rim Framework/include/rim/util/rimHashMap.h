 ...
 *  Rim Framework
 *
 *  Created by Carl Schissler on 3/6/11.
 *  Copyright 2011 Rim Software. All rights reserved.
 *
 */

#ifndef INCLUDE_RIM_HASH_MAP_H
#define INCLUDE_RIM_HASH_MAP_H
...
template < typename K, typename V, typename HashType = Hash >
class HashMap
{
	private:
		...
		//******	Private Bucket Class
			...
			class Entry
			{
				public:
					...
					RIM_INLINE Entry( HashType newKeyHash, const K& newKey, const V& newValue )
						:	next( NULL ),
							keyHash( newKeyHash ),
							key( newKey ),
							value( newValue )
	...
	public:
		...
		//******	Constructors
			...
			RIM_INLINE HashMap( Float newLoadFactor )
				:	buckets( util::allocate<Entry*>(DEFAULT_NUMBER_OF_BUCKETS) ),
					numBuckets( DEFAULT_NUMBER_OF_BUCKETS ),
					numElements( 0 ),
			...
			RIM_INLINE HashMap( HashType newNumBuckets )
			...
			RIM_INLINE HashMap( HashType newNumBuckets, Float newLoadFactor )
				:	numBuckets( math::nextPowerOf2Prime(newNumBuckets) ),
					numElements( 0 ),
					loadFactor( math::clamp( newLoadFactor, 0.1f, 2.0f ) )
			{
		...
		//******	Copy Constructor
			...
			/// Create a hash map with the specified load factor and number of buckets.
			RIM_INLINE HashMap( const HashMap& other )
		...
		//******	Assignment Operator
			...
			RIM_INLINE HashMap& operator = ( const HashMap& other )
			{
				if ( this != &other )
				{
		...
		//******	Add Method
			...
			/**
			  ...
			  */
			RIM_INLINE V* add( HashType keyHash, const K& key, const V& value )
			{
		...
		//******	Set Method
			...
			/**
			  ...
			  */
			RIM_INLINE Bool set( HashType keyHash, const K& key, const V& value )
			{
				// Compute the bucket for the new element.
				Entry** bucket = buckets + keyHash % numBuckets;
		...
		//******	Remove Methods
			...
			/**
			  ...
			  */
			RIM_INLINE Bool remove( HashType keyHash, const K& key )
			{
				// Compute the bucket for the new element.
				Entry** bucket = buckets + keyHash % numBuckets;
			...
			RIM_INLINE Bool remove( HashType keyHash, const K& key, const V& value )
			{
				// Compute the bucket for the new element.
				Entry** bucket = buckets + keyHash % numBuckets;
				Entry** previousNext = bucket;
		...
		//******	Contains Methods
			...
			RIM_INLINE Bool find( HashType keyHash, const K& key, V*& value ) 
			{
			...
			/// Query whether or not the specified key is contained in a hash map.
			RIM_INLINE Bool find( HashType keyHash, const K& key, const V*& value ) const
			{
				// Compute the bucket for the query.
				Entry* entry = *(buckets + keyHash % numBuckets);
			...
			/// Query whether or not the specified key is contained in a hash map.
			RIM_INLINE Bool contains( HashType keyHash, const K& key ) const
			{
				// Compute the bucket for the query.
				Entry* entry = *(buckets + keyHash % numBuckets);
			...
			RIM_INLINE Bool contains( HashType keyHash, const K& key, const V& value ) const
			{
				// Compute the bucket for the query.
				Entry* entry = *(buckets + keyHash % numBuckets);
		...
		//******	Get Methods
			...
			/**
			  ...
			  * returned.
			  */
			RIM_INLINE V* get( HashType keyHash, const K& key )
			{
				// Compute the bucket for the query.
				Entry* entry = *(buckets + keyHash % numBuckets);
			...
			/**
			  ...
			  * returned.
			  */
			RIM_INLINE const V* get( HashType keyHash, const K& key ) const
			{
				// Compute the bucket for the query.
				Entry* entry = *(buckets + keyHash % numBuckets);
			...
			class Iterator
			{
				public:
					...
					//	Constructor
						...
						RIM_INLINE Iterator( HashMap& newHashMap )
							:	hashMap( newHashMap ),
					...
					//	Public Methods
						...
						RIM_INLINE HashType getKeyHash() const
						{
			...
			class ConstIterator
			{
				public:
					...
					//	Constructor
						...
						/// Create a new hash map iterator for the specified hash map.
						RIM_INLINE ConstIterator( const HashMap& newHashMap )
							:	hashMap( newHashMap ),
								currentBucket( newHashMap.buckets ),
								bucketsEnd( newHashMap.buckets + newHashMap.numBuckets )
						{
					...
					//	Public Methods
						...
						/// Return the value of the key-value pair pointed to by the iterator.
						RIM_INLINE const V& operator * () const
						{
							return currentEntry->value;
						}
						...
						/// Get the value of the key-value pair pointed to by the iterator.
						RIM_INLINE const V& getValue() const
						{
							return currentEntry->value;
						}
						...
						/// Get the key of the key-value pair pointed to by the iterator.
						RIM_INLINE const K& getKey() const
						{
							return currentEntry->key;
						}
	...
	private:
		...
		//******	Private Static Data Members
			...
			static const Size DEFAULT_NUMBER_OF_BUCKETS = 19;
...
const Float		HashMap<K,V,HashType>:: DEFAULT_LOAD_FACTOR = 0.5f;
