/*
 *  rimSoundReverbFilter.h
 *  Rim Sound
 *
 *  Created by Carl Schissler on 4/18/13.
 *  Copyright 2013 __MyCompanyName__. All rights reserved.
 *
 */

#ifndef INCLUDE_RIM_SOUND_REVERB_FILTER_H
#define INCLUDE_RIM_SOUND_REVERB_FILTER_H
...
class ReverbFilter : public SoundFilter
{
	public:
		...
		//******	Wet Gain Accessor Methods
			...
			/**
			  ...
			  */
			RIM_INLINE Gain getWetGain() const
			{
			...
			/**
			  ...
			  * signal before it is mixed with input signal.
			  */
			RIM_INLINE Gain getWetGainDB() const
			{
				return util::linearToDB( targetWetGain );
			}
			...
			/**
			  ...
			  * signal before it is mixed with input signal.
			  */
			RIM_INLINE void setWetGain( Gain newWetGain )
			{
				lockMutex();
			...
			/**
			  ...
			  * signal before it is mixed with input signal.
			  */
			RIM_INLINE void setWetGainDB( Gain newDBWetGain )
			{
				lockMutex();
				targetWetGain = util::dbToLinear( newDBWetGain );
				unlockMutex();
			}
		...
		//******	Dry Gain Accessor Methods
			...
			/**
			  ...
			  * signal before it is mixed with affected signal.
			  */
			RIM_INLINE void setDryGainDB( Gain newDBDryGain )
			{
				lockMutex();
				targetDryGain = util::dbToLinear( newDBDryGain );
				unlockMutex();
			}
		...
		//******	Reverb Density Accessor Methods
			...
			/**
			  ...
			  */
			RIM_INLINE Float getDensity() const
			{
				return density;
			}
			...
			/**
			  ...
			  */
			RIM_INLINE void setDensity( Float newDensity )
			{
				lockMutex();
		...
		//******	Filter Attribute Accessor Methods
			...
			/**
			  * This method returns the value SoundFilterCategory::REVERB.
			  */
	...
	private:
		...
		//******	Private Filter Processing Methods
			...
			RIM_INLINE Float getFeedbackGainForRT60( Float delayTime, Float rt60 )
			{
		...
		//******	Private Class Declaration
			...
			/// A class which encapsulates information about a single all-pass or comb filter.
			class DelayFilter
			{
				public:
					
					RIM_INLINE DelayFilter()
					{
					}
...
#endif // INCLUDE_RIM_SOUND_REVERB_FILTER_H
