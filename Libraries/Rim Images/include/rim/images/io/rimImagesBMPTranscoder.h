...
#ifndef INCLUDE_RIM_IMAGES_BMP_TRANSCODER_H
#define INCLUDE_RIM_IMAGES_BMP_TRANSCODER_H
...
//********************************************************************************
//********************************************************************************
/// A class which handles encoding and decoding image data to/from the windows bitmap format (.bmp)
class BMPTranscoder : public ImageTranscoder
{
	public:
	...
	private:
		...
		//******	Private Class Declarations
			...
			class BMPDataInputStream;
			
			
			
			
			/// A class which handles streaming writes to a memory data buffer.
			class BMPDataOutputStream;
			
			
			
			
			/// A structure which contains the information in a BMP header.
			struct BMPHeader;
		...
		//******	Private Decoding Helper Methods
			...
			static Bool decodeFileHeader( const UByte* headerData, BMPHeader& header );
			
			
			static Bool decodeInfoHeader( const UByte* headerData, BMPHeader& header );
			
			
			static Bool checkBitsPerPixel( Size bitsPerPixel, Size headerSize );
			
			
			static Bool checkCompressionFormat( UInt compressionFormat, Size headerSize );
			
			
			static Bool getHeaderPixelFormat( const BMPHeader& header, PixelFormat& pixelType );
			
			
			static Bool encodeHeader( const BMPHeader& header, UByte* headerData );
			
			
			static Bool decodeColorMap( const UByte* colorMapData, const BMPHeader& header, UByte*& colorMap );
		...
		//******	Private Pixel Reading Methods
			...
			RIM_INLINE static UInt16 readUInt16( const UByte* data );
			RIM_INLINE static UInt32 readUInt32( const UByte* data );
			RIM_INLINE static Int32 readInt32( const UByte* data );
			...
			RIM_INLINE static Float convertFP1616ToFloat( UInt32 fp1616 )
			{
				return Float(fp1616) / Float(UInt32(1) << 16);
			}
...
#endif // INCLUDE_RIM_IMAGES_BMP_TRANSCODER_H
